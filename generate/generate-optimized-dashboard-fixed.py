...
Fixed to properly handle code context structure from add-code-context.py
"""
...
<body>
    ...
    <script>
        ...
        function showIssueDetails(issue) {{
            ...
            // Add code context if available (fixed to handle the correct structure)
        ...
        function getInlineCode(issue) {{
            // Fixed to handle the correct structure from add-code-context.py
            if (!issue.code_context || !issue.code_context.lines) return [];
            
            const lines = issue.code_context.lines;
            const targetLine = parseInt(issue.line || 0);
